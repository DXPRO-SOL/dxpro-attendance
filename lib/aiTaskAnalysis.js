...
async function analyzeTask(task) {
  const now = new Date();
  const updatedAt = task.githubUpdatedAt
    ? new Date(task.githubUpdatedAt)
    : null;
  const daysSince = updatedAt ? Math.floor((now - updatedAt) / 86400000) : null;
  const isStale =
    task.state === "open" && daysSince !== null && daysSince >= 30;
  ...
  try {
    ...
    return {
      priority: ["high", "medium", "low"].includes(parsed.priority)
        ? parsed.priority
        : "medium",
      difficulty: ["hard", "medium", "easy"].includes(parsed.difficulty)
        ? parsed.difficulty
        : "medium",
      isStale,
      suggestion:
...
function fallbackAnalysis(task, isStale, daysSince) {
  ...
  if (
    labels.some((l) =>
      ["bug", "critical", "urgent", "security", "hotfix", "blocker"].includes(
        l,
      ),
    ) ||
    ...
    labels.some((l) =>
      ["documentation", "docs", "typo", "minor", "chore"].includes(l),
    ) ||
  ...
  if (
    labels.some((l) =>
      ["epic", "architecture", "refactor", "breaking"].includes(l),
    ) ||
    ...
    labels.some((l) =>
      ["good first issue", "easy", "trivial", "simple"].includes(l),
    ) ||
  ...
  if (isStale) {
    ...
    suggestion =
      "優先度が高いタスクです。早急に着手・対応することをお勧めします。";
  } else if (difficulty === "hard") {
    suggestion =
      "複雑度が高いタスクです。細かいサブタスクに分解して取り組むと進めやすくなります。";
  } else {
