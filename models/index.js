...
const SemiAnnualFeedback = mongoose.model(
  "SemiAnnualFeedback",
  SemiAnnualFeedbackSchema,
);
const PretestSubmission = mongoose.model(
  "PretestSubmission",
  PretestSubmissionSchema,
);
...
const ApprovedLocation = mongoose.model(
  "ApprovedLocation",
  ApprovedLocationSchema,
);
...
const IntegrationConfig = mongoose.model(
  "IntegrationConfig",
  IntegrationConfigSchema,
);

// ─── ユーザー別タスク接続設定 ──────────────────────────────────────────────
// タスク管理（GitHub / JIRA / Backlog）の接続設定をログインユーザー毎に保持する
const UserTaskConfigSchema = new mongoose.Schema({
  userId: { type: mongoose.Schema.Types.ObjectId, ref: "User", required: true },
  service: { type: String, required: true }, // 'github' | 'jira' | 'backlog'
  enabled: { type: Boolean, default: false },
...
// ─── ユーザー別タスク期限日ローカル上書き ───────────────────────────────────
// 外部ツールの期限日をアプリ内でローカル上書きして管理する
const TaskDueDateSchema = new mongoose.Schema({
  userId: { type: mongoose.Schema.Types.ObjectId, ref: "User", required: true },
  service: { type: String, required: true }, // 'github' | 'jira' | 'backlog'
  taskId: { type: String, required: true }, // rawId（外部ツールのタスクID）
  dueDate: { type: String, default: "" }, // 'YYYY-MM-DD'（空=クリア済み）
  updatedAt: { type: Date, default: Date.now },
  updatedBy: { type: mongoose.Schema.Types.ObjectId, ref: "User" },
});
