...
> 更新日: 2026-05-12（実装確認済み）  
> 関連ファイル: `routes/workflow.js`（1963行）、`services/workflow-engine.js`  
> ステータス: **実装済み**
...
| URL                 | 概要                                       | アクセス権   |
| ------------------- | ------------------------------------------ | ------------ |
| GET `/workflow`     | ワークフロー一覧（SPA画面 + モーダル形式） | requireLogin |
| GET `/workflow/:id` | `/workflow?open=:id` にリダイレクト        | requireLogin |
...
| GET      | `/api/workflow`                  | ワークフロー一覧JSON（タブ/フィルタ/ページング）    | requireLogin                      |
| POST     | `/api/workflow`                  | ワークフロー新規作成・申請                          | requireLogin                      |
| GET      | `/api/workflow/employees/search` | 社員名サジェスト（`?q=`）                           | requireLogin                      |
...
| PUT      | `/api/workflow/:id`              | 下書き/差し戻し後の更新・再申請                     | requireLogin（申請者・admin）     |
| POST     | `/api/workflow/:id/submit`       | 下書きを申請状態へ変更（PUT の submit=true に転送） | requireLogin                      |
| POST     | `/api/workflow/:id/approve`      | 承認                                                | requireLogin（該当承認者・admin） |
| POST     | `/api/workflow/:id/return`       | 差し戻し                                            | requireLogin（該当承認者・admin） |
| POST     | `/api/workflow/:id/reject`       | 却下                                                | requireLogin（該当承認者・admin） |
| GET      | `/api/workflow/:id/history`      | 承認履歴取得                                        | requireLogin（関係者）            |
| GET      | `/api/workflow/:id/comments`     | コメント一覧取得                                    | requireLogin（関係者）            |
| POST     | `/api/workflow/:id/comments`     | コメント投稿（関係者に通知）                        | requireLogin（関係者）            |
| DELETE   | `/api/workflow/:id`              | 論理削除（draft のみ）                              | requireLogin（申請者・admin）     |
| POST     | `/api/workflow/upload`           | 添付ファイルアップロード（最大10MB）                | requireLogin                      |

### 申請種別（applicationTypes）

```javascript
["経費", "備品購入", "内部契約", "外部契約", "その他"];
```

### 一覧タブ（GET /api/workflow）

| tab                  | 表示内容                                        |
| -------------------- | ----------------------------------------------- |
| `mine`（デフォルト） | 自分の申請（全ステータス）                      |
| `approving`          | 自分が承認者として関与している申請（submitted） |
| `done`               | 自分が関与した承認済み申請                      |
| `all`                | 全件（管理者専用）                              |
...
<div class="wf-tabs">
  ...
  <button class="wf-tab" data-tab="pending">
    承認待ち <span id="pendingCount" class="badge"></span>
  </button>
...
function renderDynamicFields(fields) {
  return fields
    .map((f) => {
      switch (f.type) {
        case "text":
          return `<input type="text" name="${f.key}" placeholder="${f.placeholder}" ${f.required ? "required" : ""}>`;
        case "number":
          return `<input type="number" name="${f.key}" ${f.required ? "required" : ""}>`;
        case "date":
          return `<input type="date" name="${f.key}" ${f.required ? "required" : ""}>`;
        case "textarea":
          return `<textarea name="${f.key}" ${f.required ? "required" : ""}></textarea>`;
        case "select":
          return `<select name="${f.key}">${f.options.map((o) => `<option value="${o.value}">${o.label}</option>`).join("")}</select>`;
        case "stamp":
          return `<div class="stamp-field" data-key="${f.key}"><img src="/inkan.png" class="stamp-img"></div>`;
        case "currency":
          return `<input type="number" name="${f.key}" step="1" min="0" class="currency-input">`;
        default:
          return "";
      }
    })
...
async function doReturn() {
  ...
  const returnToStep =
    parseInt(document.getElementById("returnToStep").value) || 0;
...
async function downloadCsv() {
  ...
  a.href = url;
  a.download = `workflow_${year}_${month}.csv`;
  a.click();
}
```
...
| docs/18\_スケジューラ機能（scheduler）.md | 本仕様書の構成テンプレート                     |
| docs/19\_ワークフロー機能（workflow）.md  | 本仕様書                                       |

---

_このドキュメントは開発チームへの実装指示書です。実装完了後は本ドキュメントのステータスを「実装済み」に更新してください。_
