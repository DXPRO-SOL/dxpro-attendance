...
const { User, Employee, Task, TaskConfig, GitHubMapping } = require('../models');
...
async function syncGitHub(cfg) {
    ...
    const loginMap = {};
    for (const x of maps) if (x.githubLogin) loginMap[x.githubLogin.toLowerCase()] = x.userId;
    let totalSynced = 0;
    for (const repo of cfg.repos) {
        try {
            ...
            for (const issue of issues) {
                ...
                const assignedUserIds = assigneeLogins.map(l => loginMap[l.toLowerCase()]).filter(Boolean);
                const labels = (issue.labels || []).map(l => l.name);
                const state = issue.state === 'closed' ? 'closed' : 'open';
                await Task.findOneAndUpdate(
                    { repoFullName: repo, githubId: issue.id, taskType: 'issue' },
                    {
                        source: 'github', number: issue.number, title: issue.title,
                        body: issue.body || '', state, url: issue.url, htmlUrl: issue.html_url,
                        assigneeLogins, assignedUserIds, labels,
            ...
            for (const pr of prs) {
                ...
                const assignedUserIds = allLogins.map(l => loginMap[l.toLowerCase()]).filter(Boolean);
                const labels = (pr.labels || []).map(l => l.name);
                const state = pr.merged_at ? 'merged' : pr.state === 'closed' ? 'closed' : 'open';
                await Task.findOneAndUpdate(
                    { repoFullName: repo, githubId: pr.id, taskType: 'pr' },
                    {
                        source: 'github', number: pr.number, title: pr.title,
                        body: pr.body || '', state, url: pr.url, htmlUrl: pr.html_url,
                        assigneeLogins: allLogins, assignedUserIds, labels,
...
async function syncJira(cfg) {
    ...
    const maps = await GitHubMapping.find();
    const jiraMap = {};
    for (const x of maps) if (x.jiraAccountId) jiraMap[x.jiraAccountId] = x.userId;
    const base = cfg.jiraUrl.replace(/\/$/, '');
    const projectFilter = (cfg.jiraProjectKeys || []).length > 0
        ? `project in (${cfg.jiraProjectKeys.join(',')}) AND `
        : '';
    const jql = `${projectFilter}ORDER BY updated DESC`;
    let startAt = 0, totalSynced = 0;
    while (true) {
        const url = `${base}/rest/api/3/search?jql=${encodeURIComponent(jql)}&startAt=${startAt}&maxResults=100` +
        ...
        for (const issue of data.issues) {
            ...
            const assignedUserIds = accountId && jiraMap[accountId] ? [jiraMap[accountId]] : [];
            const state = f.status?.statusCategory?.key === 'done' ? 'closed' : 'open';
            const projectKey = issue.key.split('-')[0];
            const body = f.description ? extractAdfText(f.description) : '';
            await Task.findOneAndUpdate(
                { repoFullName: projectKey, githubId: parseInt(issue.id), taskType: 'issue' },
                {
                    source: 'jira',
                    ...
                    assignedUserIds, labels: f.labels || [], milestone: '',
...
async function syncBacklog(cfg) {
    ...
    const maps = await GitHubMapping.find();
    const backlogMap = {};
    for (const x of maps) if (x.backlogUserId) backlogMap[String(x.backlogUserId)] = x.userId;
    ...
    for (const projectId of targets) {
        let offset = 0;
        while (true) {
            const params = new URLSearchParams({ apiKey: cfg.backlogApiKey, count: '100', offset: String(offset) });
            ...
            for (const issue of issues) {
                ...
                const assignedUserIds = assigneeId && backlogMap[assigneeId] ? [backlogMap[assigneeId]] : [];
                const closedStatuses = ['完了', '処理済み', 'Resolved', 'Closed', 'Done'];
                const state = closedStatuses.includes(issue.status?.name || '') ? 'closed' : 'open';
                const repoKey = `backlog/${issue.projectId || projectId}`;
                await Task.findOneAndUpdate(
                    { repoFullName: repoKey, githubId: issue.id, taskType: 'issue' },
                    {
                        source: 'backlog', number: issue.id,
                        title: issue.summary || '(タイトルなし)',
                        body: issue.description || '', state,
                        ...
                        closedAt: state === 'closed' && issue.updated ? new Date(issue.updated) : null,
                        lastSyncedAt: new Date()
                    },
                    { upsert: true, new: true, setDefaultsOnInsert: true }
                );
...
async function syncAllTasks() {
    try {
        ...
        if (!cfg) { console.log('[tasks/sync] 設定なし'); return { synced: 0 }; }
...
function stateLabel(t) {
    if (t.state === 'merged')  return `<span class="task-badge badge-merged">マージ済</span>`;
    if (t.state === 'closed')  return `<span class="task-badge badge-closed">クローズ</span>`;
...
function priorityLabel(p) {
    const map = { urgent:'🔴 緊急', high:'🟠 高', medium:'🟡 中', low:'🟢 低', '':'' };
    return map[p] || '';
}
...
function buildAttachHtml(attachments) {
    ...
    return attachments.map(a => {
        const isImg = /\.(jpe?g|png|gif|webp)$/i.test(a.originalName || a.filename || '');
...
router.get('/tasks', requireLogin, async (req, res) => {
    try {
        const { state = 'all', type = 'all', stagnant = '', q = '', page = '1' } = req.query;
        const myMapping = await GitHubMapping.findOne({ userId: req.session.userId });
        ...
        const html = buildListPage({ tasks, total, pageNum, pageSize, state, type, stagnant, q, lastSync, cfg: cfg || {}, isAdmin, myMapping, req });
...
router.post('/tasks/sync', requireLogin, async (req, res) => {
    try {
        ...
        const hasPersonal = userCfg?.source && (
        ...
        if (hasPersonal) {
            ...
            // 個人の最終同期日時を更新
            await GitHubMapping.findOneAndUpdate({ userId }, { lastSyncedAt: new Date() });
            result = { synced };
        } else {
...
router.post('/tasks/ai-analyze', requireLogin, async (req, res) => {
    try {
        ...
        if (taskId) {
            ...
            if (!t) return res.status(404).json({ ok: false, error: 'タスクが見つかりません' });
            tasks = [t];
        } else {
        ...
        if (!tasks.length) return res.json({ ok: true, result: '分析対象タスクがありません。' });
        ...
        const response = await client.chat.completions.create({
            ...
            messages: [{
                role: 'system',
                content: '日本語でタスクを分析するアシスタントです。優先度・難易度・滞留リスクを分析し、担当者へのアドバイスを提供してください。'
            }, {
                role: 'user',
...
router.post('/tasks/admin/config', requireLogin, async (req, res) => {
    if (!req.session.isAdmin) return res.status(403).send('管理者権限が必要です');
    try {
        const { source, githubToken, repos,
                jiraUrl, jiraEmail, jiraApiToken, jiraProjectKeys,
                backlogSpaceId, backlogApiKey, backlogProjectIds } = req.body;
        ...
        const update = {
            ...
            jiraProjectKeys:  (jiraProjectKeys || '').split('\n').map(r => r.trim()).filter(Boolean),
            backlogSpaceId:   (backlogSpaceId || '').trim(),
            backlogProjectIds:(backlogProjectIds || '').split('\n').map(r => r.trim()).filter(Boolean),
            updatedBy: req.session.userId,
            updatedAt: new Date()
        };
        // 秘密情報：空欄の場合は既存値を保持
        if (githubToken  && githubToken.trim())  update.githubToken  = githubToken.trim();
        if (jiraApiToken && jiraApiToken.trim()) update.jiraApiToken = jiraApiToken.trim();
        if (backlogApiKey && backlogApiKey.trim()) update.backlogApiKey = backlogApiKey.trim();
...
router.get('/tasks/admin/mapping', requireLogin, async (req, res) => {
    ...
    try {
        ...
        const allMappings = {};
        for (const m of mappings) allMappings[String(m.userId)] = {
        ...
        renderPage(req, res, 'タスク管理 - ユーザーマッピング', 'ユーザーマッピング', html);
    } catch (e) {
        ...
        for (const userId of allUserIds) {
            ...
            if (githubLogin || jiraAccountId || backlogUserId) {
                ...
                await GitHubMapping.findOneAndUpdate(
                    { userId },
                    { userId, employeeId: emp?._id, githubLogin, jiraAccountId, backlogUserId, updatedAt: new Date() },
                    { upsert: true }
                );
            } else {
...
router.post('/tasks/settings', requireLogin, async (req, res) => {
    try {
        const userId = req.session.userId;
        const {
            source, githubLogin, githubToken, repos,
            jiraAccountId, jiraUrl, jiraEmail, jiraApiToken, jiraProjectKeys,
            backlogUserId, backlogSpaceId, backlogApiKey, backlogProjectIds
        } = req.body;
        ...
        updates.githubToken  = (githubToken  && githubToken.trim())  ? githubToken.trim()  : (existing.githubToken  || '');
        updates.jiraApiToken = (jiraApiToken && jiraApiToken.trim()) ? jiraApiToken.trim() : (existing.jiraApiToken || '');
        updates.backlogApiKey= (backlogApiKey&& backlogApiKey.trim())? backlogApiKey.trim(): (existing.backlogApiKey|| '');

        await GitHubMapping.findOneAndUpdate({ userId }, updates, { upsert: true, new: true });
...
router.post('/tasks/settings/disconnect', requireLogin, async (req, res) => {
    try {
        ...
        await GitHubMapping.findOneAndUpdate(
            { userId },
            {
                source: '', githubToken: '', githubLogin: '', repos: [],
                jiraAccountId: '', jiraUrl: '', jiraEmail: '', jiraApiToken: '', jiraProjectKeys: [],
                backlogUserId: '', backlogSpaceId: '', backlogApiKey: '', backlogProjectIds: [],
                updatedAt: new Date()
            },
...
router.post('/tasks/:id/comment', requireLogin,
    upload.array('attachments', 5),
    async (req, res) => {
        try {
            const task = await Task.findById(req.params.id);
            if (!task) return res.status(404).json({ ok: false, error: 'タスクが見つかりません' });
            ...
            if (!text) return res.status(400).json({ ok: false, error: 'コメント内容を入力してください' });

            // メンション解析
            const mentionMatches = text.matchAll(/@([A-Za-z0-9_\-\.]+)/g);
            const mentionUserIds = [];
            const allUsers = await User.find({}, { _id:1, username:1 }).lean();
            const userMap  = Object.fromEntries(allUsers.map(u => [u.username.toLowerCase(), String(u._id)]));
            ...
            res.status(500).json({ ok: false, error: 'コメントの保存に失敗しました' });
        }
    }
);

// ─── POST /tasks/:id/comment/:cid/delete — コメント削除 ──────────
router.post('/tasks/:id/comment/:cid/delete', requireLogin, async (req, res) => {
    try {
        const task = await Task.findById(req.params.id);
        if (!task) return res.status(404).json({ ok: false });

        const comment = task.comments.id(req.params.cid);
        if (!comment) return res.status(404).json({ ok: false, error: 'コメントが見つかりません' });
...
router.post('/tasks/:id/comment/:cid/edit', requireLogin,
    ...
    async (req, res) => {
        try {
            ...
            const text = (req.body.text || '').trim();
            if (!text) return res.status(400).json({ ok: false, error: 'テキストが必要です' });

            // 削除対象の既存添付
            const deleteIds = JSON.parse(req.body.deleteAttachments || '[]');
            const remaining = (comment.attachments || []).filter(a => !deleteIds.includes(String(a._id)));
            for (const a of (comment.attachments || [])) {
...
function buildListPage({ tasks, total, pageNum, pageSize, state, type, stagnant, q, lastSync, cfg, isAdmin, myMapping, req }) {
    const totalPages = Math.ceil(total / pageSize);
    const saved = req.query.synced ? `<div style="background:#d1fae5;color:#065f46;padding:10px 16px;border-radius:6px;margin-bottom:16px;">✅ 同期が完了しました</div>` : '';

    const sourceLabel = { github: '🐙 GitHub', jira: '🔵 Jira', backlog: '🟢 Backlog' };
    ...
    // 全ユーザー向けの「マイ設定」バー
    const mySettingsBadge = myMapping && (myMapping.githubLogin || myMapping.jiraAccountId || myMapping.backlogUserId)
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:12px 20px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            ...
            ${isAdmin ? `<a href="/tasks/admin/config" style="padding:7px 14px;background:#e5e7eb;color:#374151;border-radius:6px;text-decoration:none;font-size:13px;font-weight:600;">🔧 連携設定</a>
    ...
    `).join('') : `<tr><td colspan="6"><div class="empty-state"><i class="fa-solid fa-inbox"></i>タスクがありません</div></td></tr>`;

    const pagination = Array.from({length: totalPages}, (_, i) => i+1).map(p =>
    ...
    <div class="task-wrap">
        ...
        <div class="task-card">
            ...
            <div class="sync-bar">
                ...
                ${myMapping && (myMapping.githubLogin || myMapping.jiraAccountId || myMapping.backlogUserId)
...
function buildDetailPage(task, allUsers, req, isAdmin) {
    const isMine = (task.assignedUserIds || []).map(String).includes(String(req.session.userId));
    const canComment = isMine || isAdmin;

    const commentsHtml = task.comments.length ? task.comments.map(c => {
        const isMineComment = String(c.authorId) === String(req.session.userId);
        const attachHtml    = buildAttachHtml(c.attachments);
        const editedMark    = c.editedAt ? `<span style="font-size:10px;color:#aaa;margin-left:4px;">(編集済)</span>` : '';
        return `
    ...
    }).join('') : `<p style="color:#aaa;font-size:13px;">コメントはまだありません。</p>`;

    const userListJson = JSON.stringify(allUsers.map(u => u.username));
    const mentionUsersJson = JSON.stringify(allUsers.map(u => ({ id: String(u._id), name: u.username })));
    ...
    <div class="task-wrap">
        ...
        <div class="task-card">
            ...
            </div>` : `<p style="font-size:13px;color:#888;">このタスクにコメントするには担当者またはAdmin権限が必要です。</p>`}
        </div>
...
function buildConfigPage(cfg, req) {
    const saved = req.query.saved ? `<div style="background:#d1fae5;color:#065f46;padding:10px 16px;border-radius:6px;margin-bottom:16px;">✅ 設定を保存しました</div>` : '';
    const source      = cfg.source || 'github';
...
function buildMappingPage(employees, users, allMappings, req) {
    const saved = req.query.saved ? `<div style="background:#d1fae5;color:#065f46;padding:10px 16px;border-radius:6px;margin-bottom:16px;">✅ マッピングを保存しました</div>` : '';
...
function buildMySettingsPage(mapping, req) {
    const saved         = req.query.saved         ? `<div style="background:#d1fae5;color:#065f46;padding:10px 16px;border-radius:6px;margin-bottom:16px;">✅ 設定を保存しました</div>` : '';
    const disconnected   = req.query.disconnected  ? `<div style="background:#fef3c7;color:#92400e;padding:10px 16px;border-radius:6px;margin-bottom:16px;">🔌 接続設定を解除しました</div>` : '';
