...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      sendEmailToUser(r.userId, {
        subject: `【NOKORI残業申請】残業${isPre ? "事前" : "事後"}申請が承認されました`,
        text: `残業申請が承認されました。\n\n日付: ${moment.tz(r.date, "Asia/Tokyo").format("YYYY/MM/DD")}\n時間: ${r.startTime}～${r.endTime} (${r.hours}h)\n\n${process.env.APP_URL || ""}/overtime`,
      }).catch(() => {});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      sendEmailToUser(r.userId, {
        subject: `【NOKORI残業申請】残業${isPre ? "事前" : "事後"}申請が却下されました`,
        text: `残業申請が却下されました。\n\n日付: ${moment.tz(r.date, "Asia/Tokyo").format("YYYY/MM/DD")}\n時間: ${r.startTime}～${r.endTime}${r.rejectReason ? "\n理由: " + r.rejectReason : ""}\n\n${process.env.APP_URL || ""}/overtime`,
      }).catch(() => {});
      res.redirect("/admin/overtime?status=pending");
    } catch (err) {
      console.error(err);
