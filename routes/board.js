...
// 旧「リンク集」(/links) はナレッジ管理へ統合されたため routes/knowledge.js でリダイレクトを処理する。
// 互換のため board.js 側のハンドラは削除済み。
/* removed legacy /links handler — see routes/knowledge.js
    const html = `
                  ...
                  comments.length
                    ? comments
                        .map((c) => {
                          ...
                          const canManage =
                            isMyComment || req.session.user?.isAdmin;
                          // @メンションをハイライト
...
router.get("/board/:id/edit", requireLogin, async (req, res) => {
  ...
  if (
    !req.session.user.isAdmin &&
    req.session.user._id != post.authorId.toString()
  ) {
    return res.status(403).send("権限がありません");
  }
