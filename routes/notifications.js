...
async function createNotification({ userId, type, title, body, link, fromUserId, fromName, meta }) {
    try {
        ...
        if (global.io) {
            ...
            const userDoc = await User.findById(userId).select('preferredLang').lean();
            const lang = (userDoc && userDoc.preferredLang) || 'ja';
            const raw = { type, title, body: body || '', link: link || '', fromName: fromName || '', meta: meta || {} };
            const localized = localizeNotif(raw, lang);
            global.io.to('u_' + String(userId)).emit('notification_new', {
                type, title: localized.title, body: localized.body, link: link || '', fromName: fromName || '',
            });
        }
    } catch (e) {
...
function localizeNotif(n, lang) {
    ...
    switch (n.type) {
        case 'goal_deadline': {
            ...
            const urgency = (diffDays != null && diffDays <= 1)
                ? t('notification.goal_urgency_tomorrow', lang)
                : t('notification.goal_urgency_days', lang, { days: diffDays || '?' });
            title = t('notification.goal_deadline_title', lang, { urgency });
            break;
        }
        case 'attendance_missing':
            title = t('notification.attendance_missing_title', lang, { date: meta.date || '' });
        ...
        case 'schedule_reminder': {
            const schedTitle = meta.schedTitle || n.title.replace(/^[⏰\s]*5分後に開始:\s*/, '').trim();
            title = t('notification.schedule_reminder_title', lang, { title: schedTitle || '' });
            break;
        }
        case 'leave_approved':
        ...
        case 'comment':
            ...
            break;
        case 'reaction':
            title = t('notification.reaction_title', lang, { name: n.fromName || '' });
            break;
        case 'mention':
...
router.get('/notifications', requireLogin, async (req, res) => {
    try {
        ...
        // 日付フォーマット用ロケールマップ
        const DATE_LOCALE_MAP = { ja: 'ja-JP', en: 'en-US', vi: 'vi-VN', ko: 'ko-KR', zh: 'zh-CN' };
        const dateLocale = DATE_LOCALE_MAP[lang] || 'ja-JP';

        renderPage(req, res, t('notification.page_title', lang), t('notification.page_heading', lang), `
            <style>
            ...
            <div class="notif-list">
                ...
                ` : localizedItems.map(n => {
                    const icon = typeIcon[n.type] || '📌';
                    const date = new Date(n.createdAt).toLocaleString(dateLocale, { month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit' });
