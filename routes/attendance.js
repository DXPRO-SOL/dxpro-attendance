...
.container{max-width:1200px;margin:28px auto;padding:0 20px;width:100%;box-sizing:border-box}
.grid{display:grid;grid-template-columns:1fr 360px;gap:20px;min-width:0}
.grid > section,.grid > aside{min-width:0;width:100%;box-sizing:border-box}
@media(max-width:980px){ .grid{grid-template-columns:1fr} .aside{order:2} }

.panel{background:var(--card);border-radius:12px;padding:18px;box-shadow:0 8px 30px rgba(12,20,40,0.04);border:1px solid rgba(15,23,42,0.03);min-width:0;width:100%;box-sizing:border-box}
/* KPI: モバイルファースト（2列デフォルト → 大画面で4列） */
.kpis{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:18px;width:100%;box-sizing:border-box}
@media(min-width:900px){ .kpis{grid-template-columns:repeat(4,1fr);gap:16px} }
.kpi{padding:14px;border-radius:10px;background:linear-gradient(180deg,#fff,#fbfdff);display:flex;flex-direction:column;gap:8px;min-width:0;overflow:hidden;box-sizing:border-box}
...
.clock-card{flex:1;min-width:0;padding:18px;border-radius:12px;background:linear-gradient(90deg,#eef6ff,#ffffff);display:flex;flex-direction:column;gap:8px;align-items:flex-start;overflow:hidden}
...
.info-list{display:flex;gap:12px;flex:1;flex-wrap:wrap;min-width:0}
.info-item{min-width:0;flex:1 1 120px;background:linear-gradient(180deg,#fff,#fbfdff);padding:12px;border-radius:10px;box-shadow:0 6px 18px rgba(12,20,40,0.04);box-sizing:border-box}
...
@media(max-width:767px){
  ...
  /* ── レイアウト ── */
  .container{padding:0 8px;margin:10px auto;max-width:100%}
  .grid{grid-template-columns:1fr;min-width:0}
  .panel{padding:14px 12px;border-radius:10px;max-width:100%}

  /* ── KPI ── */
  .kpis{gap:8px;margin-bottom:14px}
  .kpi{padding:10px}
  .kpi .value{font-size:15px;word-break:break-all}
  .kpi .label{font-size:11px}
  .kpi .sub{font-size:11px}

  /* ── 今日の打刻エリア：縦積み ── */
  .attendance-today{flex-direction:column;align-items:stretch;gap:10px;width:100%}
  .clock-card{min-width:0;width:100%;padding:14px;box-sizing:border-box;flex:none}
  .clock-card .time{font-size:24px}

  /* ── 打刻ボタン：縦並び・全幅・タップしやすく ── */
  .actions{flex-direction:column;align-items:stretch;gap:8px;width:100%;margin-top:12px;box-sizing:border-box}
  .actions form{display:block !important;width:100%;box-sizing:border-box}
  .actions form .btn,
  .actions > .btn{display:flex !important;width:100%;justify-content:center;padding:13px 14px;font-size:14px;border-radius:10px;box-sizing:border-box}

  /* ── 情報一覧：2列グリッド ── */
  .info-list{width:100%;display:grid;grid-template-columns:1fr 1fr;gap:8px;flex:none;box-sizing:border-box}
  .info-item{min-width:0;padding:10px}
  .info-item .name{font-size:15px}

  /* ── テーブル ── */
  .table-scroll-hint{display:block;text-align:center;font-size:11px;color:#9ca3af;padding:4px 0 6px;letter-spacing:.03em}
  .table-wrap{border-radius:6px;-webkit-overflow-scrolling:touch}
  .att-table thead th{font-size:11px;padding:8px 6px;white-space:nowrap}
  .att-table tbody td{font-size:12px;padding:9px 6px}

  /* ── クイックリンク ── */
  .link-card{padding:14px 12px}
  .footer-actions{justify-content:stretch;flex-wrap:wrap}
  .footer-actions .btn{flex:1;justify-content:center;min-width:120px}

  /* ── GPS ステータス ── */
  #gpsStatus,#coGpsStatus{word-break:break-word;font-size:12px;margin-top:6px}

  /* ── パネルヘッダー ── */
  .panel-hdr-sub{display:none}
  .panel-hdr-btns .btn-label{display:none}
  .panel-hdr-btns{gap:6px}
  .panel-hdr-btns .btn{padding:9px 11px;gap:0;min-width:0}

  /* ── clock-card 内部 ── */
  .clock-inner-row{flex-wrap:wrap;gap:6px}
  .clock-inner-row .tag{align-self:flex-start}
}
</style>
</head>
<body>
<header class="header">
  ...
  <div class="header-right">
    ...
    <div class="header-spacer" style="width:12px"></div>
    ...
    <section>
      <div class="panel">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px">
          ...
          <div class="panel-hdr-btns" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            <a href="/add-attendance" class="btn btn--ghost"><i class="fa-solid fa-plus"></i><span class="btn-label"> 打刻追加</span></a>
            <a href="/attendance/bulk-register?year=${moment().tz("Asia/Tokyo").year()}&month=${moment().tz("Asia/Tokyo").month() + 1}" class="btn btn--ghost"><i class="fa-solid fa-calendar-check"></i><span class="btn-label"> 一括登録</span></a>
            ${req.session.isAdmin ? `<a href="/admin/monthly-attendance" class="btn btn--ghost"><i class="fa-solid fa-shield-halved"></i><span class="btn-label"> 管理メニュー</span></a>` : ""}
          </div>
        </div>
        ...
        <div class="kpis">
          <div class="kpi">
            ...
            <div class="value">${todayAttendance && todayAttendance.checkIn ? moment(todayAttendance.checkIn).tz("Asia/Tokyo").format("HH:mm") : "-"}</div>
            <div class="sub">出勤時間</div>
          </div>
          <div class="kpi">
            <div class="label">退勤</div>
            <div class="value">${todayAttendance && todayAttendance.checkOut ? moment(todayAttendance.checkOut).tz("Asia/Tokyo").format("HH:mm") : "-"}</div>
        ...
        <div class="attendance-today">
          <div class="clock-card">
            <div class="clock-inner-row" style="display:flex;justify-content:space-between;width:100%;align-items:center">
              <div>
              ...
              ${
                ...
                ${
                  ...
                  (!todayAttendance.lunchStart || todayAttendance.lunchEnd)
                    ? `
                  <form action="/start-lunch" method="POST"><button class="btn btn--primary" type="submit"><i class="fa-solid fa-utensils"></i> 昼休み開始</button></form>
                `
                    : ""
                }
                ${
                  todayAttendance.lunchStart && !todayAttendance.lunchEnd
                    ? `
                  <form action="/end-lunch" method="POST"><button class="btn btn--success" type="submit"><i class="fa-solid fa-handshake"></i> 昼休み終了</button></form>
                `
                    : ""
                }
      ...
      <div class="panel" style="margin-top:16px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:4px">
          ...
          <div class="panel-hdr-sub" style="color:var(--muted);font-size:13px">編集は各行の操作から</div>
        </div>
        <div class="table-scroll-hint">← 横スクロールできます →</div>
...
<script>
  ...
  function updateClocks(){
    const now = new Date();
    const opts = { hour12:false, timeZone:'Asia/Tokyo', hour:'2-digit', minute:'2-digit', second:'2-digit' };
    const hms = now.toLocaleTimeString('ja-JP', opts);
    const hm  = hms.slice(0,5); // HH:mm
    const isMobile = window.innerWidth <= 767;
    document.getElementById('main-clock').textContent = isMobile ? hm : hms;
    document.getElementById('header-clock').textContent = now.toLocaleString('ja-JP', { timeZone:'Asia/Tokyo' });
  }
...
.page-header h2 { margin:0; font-size:22px; font-weight:700; color:#0b2540; min-width:0; word-break:break-word; }
/* stats: モバイルファースト 2列 → 大画面で4列 */
.stats-row { display:grid; grid-template-columns:repeat(2,1fr); gap:10px; margin-bottom:20px; }
@media(min-width:700px){ .stats-row{grid-template-columns:repeat(4,1fr);gap:14px} }
.stat-card { background:#fff; border-radius:10px; padding:12px 14px; box-shadow:0 2px 8px rgba(0,0,0,.06); min-width:0; }
.stat-card .s-label { font-size:12px; color:#6b7280; margin-bottom:4px; }
.stat-card .s-value { font-size:20px; font-weight:800; color:#0b2540; word-break:break-all; }
.month-nav { display:flex; gap:8px; align-items:center; flex-wrap:wrap; }
.month-nav select { padding:7px 10px; border-radius:7px; border:1px solid #d1d5db; font-size:14px; flex:1; min-width:80px; }
/* テーブルラッパー: overflow-x:auto でスクロール */
/* .main/.page-content の overflow-x:hidden を上書きしてスクロールを有効化 */
.tbl-card { padding:0 !important; }
...
@media(max-width:768px){
  /* テーブルスクロールのためにoverflow制限を全階層で解除 */
  .app-wrapper { overflow-x: visible !important; }
  .main { overflow-x: visible !important; }
  .page-content { overflow-x: visible !important; }
  .page-header h2 { font-size:17px; }
  .action-row .btn { flex:1; justify-content:center; min-width:0; }
...
<!-- 勤怠テーブル -->
<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;border-radius:10px;border:1px solid #e2e8f0;background:#fff;margin-bottom:16px;box-shadow:0 1px 3px rgba(0,0,0,.08)">
    <div class="tbl-wrap" style="overflow-x:auto;-webkit-overflow-scrolling:touch;min-width:0">
        <table>
            <thead>
                <tr>
                    <th>日付</th>
