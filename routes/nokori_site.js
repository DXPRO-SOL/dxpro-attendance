...
const {
  NokoriPlan, NokoriOption, NokoriNews, NokoriFAQ,
  NokoriInquiry, NokoriDocumentRequest, NokoriEstimate, NokoriPartnerApplication,
  NokoriDemoRequest,
} = require("../models");
...
router.get("/nokori", async (req, res) => {
  ...
  const news = await NokoriNews.find({ isPublished: true }).sort({ publishedAt: -1 }).limit(5).lean();
  const body = `
<section class="nk-hero">
  <div style="max-width:860px;margin:0 auto;">
    <p style="background:rgba(255,255,255,.15);border-radius:20px;display:inline-block;padding:6px 18px;font-size:13px;margin-bottom:16px;letter-spacing:.05em;">🚀 クラウドHRシステム NOKORI</p>
    <h1>勤怠・人事・採用を<br>ひとつに、もっとシンプルに。</h1>
    <p>中小企業から大企業まで対応。導入後すぐに使えるオールインワンの人事管理クラウドシステムです。</p>
    <div class="nk-hero-actions">
      <a href="/nokori/register" class="nk-btn-lg nk-btn-white">🆓 無料で始める</a>
      <a href="/nokori/demo" class="nk-btn-lg nk-btn-ghost">📺 デモを体験する</a>
    </div>
    <p style="font-size:13px;color:rgba(255,255,255,.6);margin-top:20px;">クレジットカード不要 • 14日間無料トライアル</p>
  </div>
</section>
...
<section class="nk-section nk-section--gray">
  <div class="nk-section-inner">
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:24px;text-align:center;">
      <div><div style="font-size:44px;font-weight:800;color:#0f4c81;">5,000+</div><div style="color:#64748b;font-size:14px;">導入企業数</div></div>
      <div><div style="font-size:44px;font-weight:800;color:#0f4c81;">98%</div><div style="color:#64748b;font-size:14px;">顧客満足度</div></div>
      <div><div style="font-size:44px;font-weight:800;color:#0f4c81;">30+</div><div style="color:#64748b;font-size:14px;">搭載機能数</div></div>
      <div><div style="font-size:44px;font-weight:800;color:#0f4c81;">24/7</div><div style="color:#64748b;font-size:14px;">サポート体制</div></div>
    </div>
  </div>
</section>
...
<!-- CTA -->
<section style="background:linear-gradient(135deg,#0f4c81,#0ea5e9);padding:80px 24px;text-align:center;color:#fff;">
  <h2 style="font-size:clamp(22px,3.5vw,36px);font-weight:800;margin-bottom:16px;">今すぐNOKORIをはじめましょう</h2>
  <p style="font-size:17px;color:rgba(255,255,255,.85);margin-bottom:32px;">14日間無料トライアル。クレジットカード不要。いつでもキャンセル可能。</p>
...
<section class="nk-section">
  <div class="nk-section-inner">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:24px;">
      ${[["製造業","工場での打刻・シフト管理、多拠点の従業員を一元管理"],["IT・テック","リモートワーク管理、スキルシート管理、プロジェクト別タスク管理"],["小売・サービス","パートタイマー管理、シフト調整、給与明細のペーパーレス化"],["医療・介護","複雑なシフト管理、資格管理、コンプライアンス対応"]].map(([title,desc])=>`<div class="nk-card"><h3>${title}</h3><p>${desc}</p></div>`).join("")}
    </div>
  </div>
</section>
...
router.get("/nokori/features", (req, res) => {
  const features = [
    { icon: "⏰", name: "勤怠管理", items: ["GPS打刻", "QRコード打刻", "打刻修正申請", "残業自動計算", "月次集計・CSV出力"] },
    { icon: "💰", name: "給与計算", items: ["勤怠データ連動", "控除自動計算", "給与明細発行", "振込データ出力", "法改正自動対応"] },
    { icon: "🌴", name: "休暇・申請管理", items: ["有給休暇残日数管理", "各種申請フロー", "承認ワークフロー", "残業申請", "休暇カレンダー"] },
    { icon: "🎯", name: "目標管理（OKR）", items: ["目標設定", "進捗トラッキング", "1on1サポート", "半期評価連動", "ダッシュボード"] },
    { icon: "✅", name: "タスク管理", items: ["カンバンボード", "ガントチャート", "担当者・期限設定", "外部連携（GitHub/Jira）", "AI優先度分析"] },
    { icon: "💬", name: "チャット・コミュニケーション", items: ["DM・グループチャット", "グループ通話（WebRTC）", "ファイル添付", "既読確認", "チャットボット（AI）"] },
    { icon: "📊", name: "人事・HR", items: ["スキルシート", "日報管理", "採用前テスト", "組織図", "部署・ロール管理"] },
    { icon: "📅", name: "スケジュール管理", items: ["会議室予約", "グループカレンダー", "リマインダー通知", "通話連携", "外部カレンダー連携"] },
    { icon: "🗂️", name: "クラウドドライブ", items: ["ファイル・フォルダ管理", "同時編集", "バージョン管理", "共有リンク発行", "容量管理"] },
    { icon: "⚙️", name: "管理・セキュリティ", items: ["ロール別アクセス制御", "監査ログ", "IP制限", "SSO連携", "2段階認証"] },
  ];
  const body = `
<section style="background:#0f4c81;color:#fff;padding:70px 24px;text-align:center;">
...
router.post("/nokori/demo-request", async (req, res) => {
  try {
    ...
    await NokoriDemoRequest.create({ name, company, email, phone, employees: parseInt(employees) || 0, purpose });
    // 管理者通知
    await sendMail({
      to: process.env.ADMIN_EMAIL || process.env.MAIL_FROM || "info@nokori-hr.jp",
...
router.get("/nokori/pricing", async (req, res) => {
  ...
  if (plans.length === 0) {
    plans = [
      { _id: "starter", name: "スタータープラン", code: "starter", description: "小規模チーム向け", initialFee: 0, monthlyFee: 3980, maxUsers: 10, storageGB: 10, features: ["勤怠管理", "給与計算", "チャット", "タスク管理", "メールサポート"], isPopular: false },
      { _id: "standard", name: "スタンダードプラン", code: "standard", description: "中規模企業向け", initialFee: 50000, monthlyFee: 9800, maxUsers: 50, storageGB: 50, features: ["スタータープランの全機能", "休暇申請管理", "承認ワークフロー", "クラウドドライブ", "優先サポート"], isPopular: true },
      { _id: "professional", name: "プロフェッショナル", code: "professional", description: "大規模企業向け", initialFee: 100000, monthlyFee: 29800, maxUsers: 0, storageGB: 200, features: ["スタンダードの全機能", "無制限ユーザー", "AI機能フル活用", "専任サポート担当", "カスタマイズ対応"], isPopular: false },
    ];
  }
...
router.get("/nokori/contact", (req, res) => {
  const member = getMember(req);
  const saved = member ? { name: member.name, company: member.company, email: member.email, phone: member.phone } : {};
  const body = `
<section style="background:#0f4c81;color:#fff;padding:70px 24px;text-align:center;">
...
router.post("/nokori/contact", async (req, res) => {
  try {
    ...
    } catch (mailErr) { console.error("inquiry mail error:", mailErr.message); }
...
router.post("/nokori/document-request", async (req, res) => {
  try {
    ...
    await NokoriDocumentRequest.create({ name, company, email, phone, sentAt: new Date() });
    try {
      await sendMail({
        to: email,
      ...
      if (process.env.ADMIN_EMAIL) {
        await sendMail({ to: process.env.ADMIN_EMAIL, subject: `【NOKORI管理】資料請求: ${name} (${company})`, text: `資料請求\n氏名: ${name}\n会社: ${company}\nメール: ${email}\n電話: ${phone}` });
      }
    } catch (e) { console.error("doc-request mail:", e.message); }
...
router.get("/nokori/estimate", async (req, res) => {
  ...
  if (plans.length === 0) {
    plans = [
      { _id: "starter", name: "スタータープラン", monthlyFee: 3980, initialFee: 0 },
      { _id: "standard", name: "スタンダードプラン", monthlyFee: 9800, initialFee: 50000 },
      { _id: "professional", name: "プロフェッショナル", monthlyFee: 29800, initialFee: 100000 },
    ];
  }
...
router.post("/nokori/estimate", async (req, res) => {
  try {
    ...
    const opts = optionIds ? await NokoriOption.find({ _id: { $in: Array.isArray(optionIds) ? optionIds : [optionIds] } }).lean() : [];
    const users = parseInt(userCount) || 1;
    const optFee = opts.reduce((s, o) => s + (o.monthlyFee || 0), 0);
    const monthlyFee = plan ? (plan.monthlyFee * Math.ceil(users / 10)) + optFee : optFee;
    ...
    const doc = await NokoriEstimate.create({
      estimateNo, email, name, company,
      planId: plan ? plan._id : null,
      optionIds: opts.map(o => o._id),
      userCount: users, initialFee, monthlyFee, firstBillingAmount,
    });
    // PDF生成してメール添付
    try {
...
<p class="footer">本見積書の有効期間は発行日より30日間です。<br>NOKORI | info@nokori-hr.jp</p>
      ...
      const pdfBuf = await new Promise((resolve, reject) => {
        pdfLib.create(pdfHtml, { format: "A4", border: { top: "20mm", right: "15mm", bottom: "20mm", left: "15mm" } })
      ...
      await sendMail({
        ...
        attachments: [{ filename: `estimate-${estimateNo}.pdf`, content: pdfBuf, contentType: "application/pdf" }],
      });
...
<section class="nk-section"><div class="nk-section-inner" style="max-width:700px;">
  ...
  res.send(page("メールアドレス無断収集禁止", body, { member: getMember(req) }));
});
...
router.post("/nokori/partner", async (req, res) => {
  try {
    const { companyName, contactName, email, phone, businessType, description } = req.body;
    await NokoriPartnerApplication.create({ companyName, contactName, email, phone, businessType, description });
    try {
      await sendMail({ to: email, subject: "【NOKORI】協力会社申請を受け付けました", text: `${contactName} 様\n\n協力会社申請ありがとうございます。\n担当者より5営業日以内にご連絡いたします。\n\n--\nNOKORI チーム` });
      if (process.env.ADMIN_EMAIL) {
        await sendMail({ to: process.env.ADMIN_EMAIL, subject: `【NOKORI管理】協力会社申請: ${companyName}`, text: `会社名: ${companyName}\n担当者: ${contactName}\nメール: ${email}\n電話: ${phone}\n事業内容: ${businessType}\n内容: ${description}` });
      }
    } catch (e) { console.error("partner mail:", e.message); }
