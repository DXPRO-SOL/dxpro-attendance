...
// 1. 画面 GET /workflow
// ════════════════════════════════════════════════════════════════════════════
router.get("/workflow/:id", requireLogin, (req, res) => {
  res.redirect(`/workflow?open=${encodeURIComponent(req.params.id)}`);
});
...
router.get("/api/workflow", requireLogin, async (req, res) => {
  try {
    ...
    // キーワード検索条件を事前に構築（各タブのクエリに共通適用）
    let kwOrConditions = null;
    if (keyword && keyword.trim()) {
      ...
      const matchedIds = matchedEmps.map((e) => e.userId);
      kwOrConditions = [
        { title: kw },
        { description: kw },
        { serialNo: kw },
        ...(matchedIds.length ? [{ applicantId: { $in: matchedIds } }] : []),
      ];
      query.$and = [{ $or: kwOrConditions }];
    }
    let items = [];
    let total = 0;
    ...
    if (isAdmin) {
      ...
      if (kwOrConditions) approveQuery.$and = [{ $or: kwOrConditions }];
...
.wf-tbl-scroll .wf-table { min-width:680px; }
...
(function() {
    ...
    window.wfLoadList = async function() {
        ...
        try {
            ...
            let html = '<div class="wf-tbl-scroll"><table class="wf-table"><thead><tr>' +
    ...
    // 初期ロード（通知リンクの ?open=:id 対応）
    wfLoadList();
    const _openId = new URLSearchParams(location.search).get('open');
    if (_openId) { wfLoadList().then(() => wfOpenDetail(_openId)); }
})();
</script>
`;
}
