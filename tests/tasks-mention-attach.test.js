...
function createAttachContext() {
    ...
    return {
        ...
        removeFile,
        handleDrop,
        reset() { selectedFiles = []; chips.children = []; }
    };
}
...
test("renderMentions: @ だけで名前がない場合は変換しない", () => {
    ...
    assert.ok(!result.includes('<span class="mention">'), "@ のみは変換されないこと");
});
...
test("renderMentions: メンション名内の特殊文字はエスケープされる", () => {
    ...
    assert.ok(!result.includes('<evil>'), "メンション名内の < > がエスケープされること");
});
...
test("buildAttachHtml: 画像ファイルは <img> タグで表示する", () => {
    const result = buildAttachHtml([{ originalName: 'photo.jpg', filename: 'saved_photo.jpg' }]);
...
test("buildAttachHtml: PDF は <a> リンクで表示し img タグを含まない", () => {
    const result = buildAttachHtml([{ originalName: 'doc.pdf', filename: 'doc.pdf' }]);
...
test("buildAttachHtml: ファイル名の XSS はエスケープされる", () => {
    const result = buildAttachHtml([{ originalName: '<img src=x onerror=alert(1)>.jpg', filename: 'safe.jpg' }]);
    assert.ok(!result.includes('<img src=x'), "XSS が出力されないこと");
});

test("buildAttachHtml: ファイル名はURLエンコードされる", () => {
    const result = buildAttachHtml([{ originalName: '添付資料.pdf', filename: '添付資料.pdf' }]);
    assert.ok(result.includes('%E6%B7%BB%E4%BB%98%E8%B3%87%E6%96%99.pdf'), "日本語ファイル名がURLエンコードされること");
});
...
test("添付UI: 複数ファイルを一度に追加できる", () => {
    const ctx = createAttachContext();
    ctx.handleFileChange([makeFile('a.pdf'), makeFile('b.png'), makeFile('c.txt')]);
...
test("メンション挿入: 挿入後の元テキストの後続部分は保持される", () => {
    ...
    assert.ok(result.value.startsWith("@charlie "), "先頭が @charlie + スペースになること");
